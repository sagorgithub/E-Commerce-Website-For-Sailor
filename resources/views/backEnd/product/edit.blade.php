...
<div class="container-fluid">
  ...
  <div class="row justify-content-center">
    ...
    <div class="input-group control-group">
        <img src="{{ asset($image->image) }}" alt="Product Image" style="height: 100px; width: auto;">
        <input type="text" name="title[]" class="form-control mt-2" value="{{ $image->title }}" placeholder="Image Title">
        <input type="hidden" name="image[]" value="{{ $image->id }}">
        <div class="input-group-btn">
            <button class="btn btn-danger remove-image" type="button" data-image-id="{{ $image->id }}"><i class="fa fa-trash"></i></button>
        </div>
    </div>
    @endforeach
            ...
            @foreach($skintypes as $type)
                <div class="form-check me-3">
                    <input type="checkbox" 
                           ...
                           id="skintype_{{ $type->name }}"
                           @if(is_array($edit_data->skin_type) && in_array($type->name, $edit_data->skin_type)) checked @endif>
                    <label class="form-check-label" for="skintype_{{ $type->name }}">
                        {{ $type->name }}
                    </label>
                </div>
            @endforeach
            ...
            @foreach($skinconcerns as $concern)
                <div class="form-check me-3">
                    <input type="checkbox" 
                           ...
                           value="{{ $concern->name }}" 
                           class="form-check-input" 
                           id="skinconcern_{{ $concern->name }}"
                           @if(is_array($edit_data->skin_concern) && in_array($concern->name, $edit_data->skin_concern)) checked @endif>
                    <label class="form-check-label" for="skinconcern_{{ $concern->name }}">
                        {{ $concern->name }}
                    </label>
                </div>
            @endforeach
